...
public class DimensionValidator {
	private int m_rowsCount;
	   ...
	   public DimensionValidator(String path) throws IOException {
		    m_url = new URL(path); 
	   ...
	   public static JSONObject returnResult(Vector<String[]> results) throws JSONException{
		   ...
		   for(int i=0;i<results.size();i++){
			   ...
			   one_result.put("label", tmp[0]);
			   one_result.put("id", tmp[1]);
			   one_result.put("type", tmp[2]);
			   one_result.put("frequency", tmp[3]);
			   if(i==0){
				   try {
		   ...
		   JSONObject result = new JSONObject();
			Boolean propositionexist = false;
			   for(int i=0;i<limit;i++){
				   String[] tmp = results.get(i);
				   JSONObject one_result=  new JSONObject();
				   if(!type.toLowerCase().equals("null")&& tmp[2].toLowerCase().equals(type.toLowerCase())){
	   ...
	   public static JSONObject returnTypeProposition(Vector<String[]> results,String type) throws JSONException{
			...
			Boolean typeexist = false;
			   for(int i=0;i<results.size();i++){
				   String[] tmp = results.get(i);
				   JSONObject one_result=  new JSONObject();
				   if(tmp[2].toLowerCase().equals(type.toLowerCase())){
					   typeexist = true;
	   ...
	   public static Vector<String[]> classResult(Vector<String[]> result) throws JSONException{
		   for(int i =0;i<result.size()-1;i++){
			   for(int j = result.size()-1;j>i;j--){
				   ...
				   if(Integer.valueOf(tmp[3])>Integer.valueOf(tmp_1[3])){
					   result.set(j, tmp_1);
					   result.set(j-1,tmp);
				   }
				   else{
	   ...
	   public static Vector<String[]>identifyCommonUnit(String file,String input) throws IOException{
		   DimensionValidator monFichier = new DimensionValidator(file); 
	   ...
	   private void readFromFile(URL monURL){
		   ...
		   try {
			...
			while (tempLine != null) {
	               m_table.add(readFromLine(tempLine));
	               tempLine =m_data.readLine();
	            }		
		} catch (IOException e) {
	   ...
	   private String[] readFromLine(String tempLine) {
		      if (tempLine == null) {
		         return null;
		      }
	   ...
	   public int findRow(String input){
		   ...
		   for (int j = 0;j<this.m_table.size();j++){
			   ...
			   for (int i = 0;i<tmp.length-2;i++){
					   if(tmp[i].toLowerCase().equals(input.toLowerCase())){
						   find = true;
						   break;
					   }
				   }
